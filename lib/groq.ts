...
const INTENSITY_RULES: Record<Intensity, string> = {
  ...
  savage:
    "'개싸가지 모드'. 공격 4~5개. 대놓고 반말로 하대하고, 비꼬고, 한숨 쉬듯 무시하는 갑질 태도를 극대화한다. 상대를 아랫사람 취급한다. **단 선은 반드시 지킨다** — 욕설·비속어, 외모·나이·출신·학벌·성별 등 인신공격은 절대 금지. 무례함은 오직 '태도'(반말·하대·비아냥·한숨)로만 표현하고, 공격 대상은 여전히 '시안/일' 그 자체여야 한다. 페르소나별 집착(사장님형=윗선 핑계, 느낌형=무드, 디테일형=디테일, 레퍼런스형=레퍼+예산)은 그대로 유지하되 말투만 반말+갑질로 뒤집는다.",
};

function systemPrompt(intensity: Intensity): string {
  const registerRule =
    intensity === "savage"
      ? "**말투(register) — 개싸가지 모드**: 이번엔 모든 페르소나가 대행사에게 반말로 꺼들먹거리며 하대한다(존댓말 예외). 각 페르소나의 [집착]은 그대로 유지하되 말투만 반말+갑질로 뒤집어라. 그래도 욕설·인신공격은 금지, 공격은 '시안/일'에 대해서만."
